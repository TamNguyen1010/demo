  ...
  useEffect(() => {
    ...
    const columns: KanbanColumn[] = [
      { 
        ...
        title: 'Bản nháp', 
        color: '#6B7280', 
        projects: localProjects.filter(p => p.approval_status === 'initialized')
      },
      { 
        id: 'pending_approval', 
        title: 'Chờ phê duyệt', 
        color: '#F59E0B', 
        projects: localProjects.filter(p => p.approval_status === 'pending_approval')
      },
      { 
        id: 'approved', 
        title: 'Đã phê duyệt', 
      ...
      { 
        ...
        title: 'Đã từ chối', 
        color: '#EF4444', 
        projects: localProjects.filter(p => p.approval_status === 'rejected')
      },
      { 
        id: 'in_progress', 
        title: 'Đang thực hiện', 
        color: '#3B82F6', 
        projects: localProjects.filter(p => p.execution_status === 'in_progress')
      },
      { 
        id: 'completed', 
        title: 'Hoàn thành', 
        color: '#8B5CF6', 
        projects: localProjects.filter(p => p.execution_status === 'completed')
      },
      { 
        id: 'suspended', 
        title: 'Dừng thực hiện', 
        color: '#F97316', 
        projects: localProjects.filter(p => p.execution_status === 'suspended')
      }
    ]
            ...
            <div className="grid grid-cols-1 lg:grid-cols-7 gap-4 min-w-[1200px]">
              {kanbanColumns.map((column) => {
                ...
                return (
                  <div
                    key={column.id}
                    className={`min-h-[600px] p-4 rounded-lg border-2 border-dashed transition-all duration-200 ${
                    ...
                    <div className="space-y-3">
                      {column.projects.map((project) => (
                        <div
                          ...
                          className="bg-white p-3 rounded-lg shadow-sm border border-slate-200 hover:shadow-md transition-all duration-200 cursor-pointer w-full"
                        >
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center justify-between text-xs">
                              <span className="text-slate-600 truncate">Ngân sách:</span>
                              <span className="font-medium truncate ml-1">{formatCurrency(project.planned_budget)}</span>
                            </div>
                            <div className="flex items-center justify-between text-xs">
                              <span className="text-slate-600 truncate">Phòng ban:</span>
                              <span className="font-medium truncate ml-1">{project.department}</span>
                            </div>
                          </div>
