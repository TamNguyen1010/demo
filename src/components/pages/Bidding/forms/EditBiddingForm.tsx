...
import { BiddingPackage, TenderMethod, TenderStatus, Priority } from "@/types/bidding"
...
interface EditBiddingFormProps {
  biddingPackage: BiddingPackage
  projects: ProjectOption[]
  onSubmit: (id: number, data: Partial<BiddingPackage>) => void
  onSaveDraft: (data: Partial<BiddingPackage>) => void
  onCancel: () => void
  onViewHistory: () => void
}
...
export function EditBiddingForm({ 
  biddingPackage, 
  projects, 
  onSubmit, 
  onSaveDraft, 
  onCancel, 
  onViewHistory 
}: EditBiddingFormProps) {
  const [currentStep, setCurrentStep] = useState(1)
  const [formData, setFormData] = useState<Partial<BiddingPackage>>({
    name: biddingPackage.name,
    description: biddingPackage.description,
    project_id: biddingPackage.project_id,
    tender_method: biddingPackage.tender_method,
    estimated_value: biddingPackage.estimated_value,
    currency: biddingPackage.currency,
    start_date: biddingPackage.start_date || '',
    end_date: biddingPackage.end_date || '',
    priority: biddingPackage.priority,
    notes: biddingPackage.notes,
    tbmt_code: biddingPackage.tbmt_code,
    participant_count: biddingPackage.participant_count,
    hsmt_approval_decision: biddingPackage.hsmt_approval_decision,
    kqlcnt_approval_decision: biddingPackage.kqlcnt_approval_decision
  })
  const [errors, setErrors] = useState<Record<string, string>>({})
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [hasChanges, setHasChanges] = useState(false)
  const [originalData] = useState<Partial<BiddingPackage>>({ ...biddingPackage })
  ...
  const handleSubmit = async () => {
    if (validateStep(currentStep)) {
      ...
      try {
        await onSubmit(biddingPackage.id, formData)
      } finally {
  ...
  const updateFormData = (field: keyof BiddingPackage, value: any) => {
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 p-4 rounded-lg">
        ...
        <p className="text-sm text-blue-700">
          Mã gói thầu không thể chỉnh sửa sau khi tạo. Một số thay đổi quan trọng có thể yêu cầu phê duyệt.
        </p>
      </div>

      <div className="space-y-4">
        <div>
          <Label htmlFor="package_code" className="text-sm font-medium text-gray-600">
            Mã gói thầu (Không thể chỉnh sửa)
          </Label>
          <Input
            id="package_code"
            value={biddingPackage.package_code}
            disabled
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      <div className="bg-blue-50 p-4 rounded-lg">
        <h3 className="font-medium text-blue-900 mb-2">Thông tin bổ sung (Có thể chỉnh sửa)</h3>
        <p className="text-sm text-blue-700">
  ...
  const renderStep4 = () => (
    <div className="space-y-6">
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="font-medium text-gray-900 mb-3">Xem lại thay đổi</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {formData.project_id !== originalData.project_id && (
                  <Badge variant="outline" className="text-xs text-blue-600">
                    Đã thay đổi
                  </Badge>
                )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {formData.tender_method !== originalData.tender_method && (
                  <Badge variant="outline" className="text-xs text-blue-600">
                    Đã thay đổi
                  </Badge>
                )}
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {formData.estimated_value !== originalData.estimated_value && (
                  <Badge variant="outline" className="text-xs text-blue-600">
                    Đã thay đổi
                  </Badge>
                )}
      ...
      <div className="bg-yellow-50 p-4 rounded-lg">
        ...
        <p className="text-sm text-yellow-700 mt-1">
          Các thay đổi quan trọng sẽ được ghi log và có thể yêu cầu phê duyệt. Bạn có thể hoàn tác thay đổi trong vòng 24 giờ.
        </p>
      </div>
    </div>
  )
  ...
  const getStepTitle = (step: number) => {
    switch (step) {
      ...
      case 4: return 'Xem lại thay đổi'
      default: return ''
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      {/* Header with History and Rollback */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Chỉnh sửa gói thầu</h2>
          <p className="text-gray-600">Cập nhật thông tin gói thầu: {biddingPackage.package_code}</p>
        </div>
        <div className="flex gap-2">
        ...
        <div className="flex gap-2">
          ...
          {currentStep < totalSteps ? (
            ...
            <Button onClick={handleSubmit} disabled={isSubmitting || !hasChanges}>
              {isSubmitting ? 'Đang cập nhật...' : 'Cập nhật gói thầu'}
            </Button>
          )}
        </div>
      </div>
